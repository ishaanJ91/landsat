...
function MyMap() {
  ...
  const fetchNDVIData = async (lat, lng) => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/api/earth-engine-data`,
        {
  ...
  // Frontend: handleCheckOverpass function modification
  const handleCheckOverpass = async (lat, lng) => {
    console.log("Checking overpass for Lat:", lat, "Lng:", lng);
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/api/predict-overpass`,
        {
          latitude: lat,
          longitude: lng,
          startDate,
      ...
      console.log("Response from overpass prediction:", response.data);
      ...
      console.error("Error checking overpass:", error);
      if (error.response?.status === 401) {
        setError("Please log in to check overpass predictions");
      } else {
        setError("Failed to fetch overpass data. Please try again.");
      }
    }
  };
  ...
  const convertLatLngToPathRow = async (lat, lng) => {
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/api/convert-latlng-to-pathrow`, // Use only REACT_APP_API_URL
        {
          latitude: lat,
          longitude: lng,
        }
      );
      console.log("Path/Row Conversion:", response.data); // Log the result
      return response.data; // Return the data
    } catch (error) {
      console.error(
        "Error converting lat/lng to path/row:",
  ...
  const reverseGeocode = (lat, lng) => {
    ...
    geocoder.geocode({ location }, (results, status) => {
      if (status === "OK" && results[0]) {
        const addressDetails = results[0].address_components.reduce(
          (acc, component) => {
            if (component.types.includes("country")) {
              acc.country = component.long_name; // Extract country
            }
            if (component.types.includes("administrative_area_level_1")) {
              acc.state = component.long_name; // Extract state
            }
            return acc;
          },
          {}
        ...
        setAddressComponents({}); // Reset on failure
      }
    });
  };
  ...
  console.log("Google Maps API Key:", process.env.REACT_APP_GOOGLE_MAPS);
